...
const typeVarConstraintViolation = (
  ...
  const keys = Z.filter (k => {
    console.log ('------------------------');
    console.log (Z.chain (({path, value}) => path === k ? [value] : [], valuesAtPath));
    console.log (valuesByPath[k]);
    console.log ('========================');
...
$.Unknown = Object.assign (Object.create (Type$prototype), {
  ...
  new: ctx => typeVarMap => newTypeVarMap => cont => cont (newTypeVarMap) (ctx.value),
});
...
const NullaryType = name => url => supertypes => test => Object.assign (Object.create (Type$prototype), {
  ...
  new: ctx => typeVarMap => newTypeVarMap => cont => {
    if (test (ctx.value)) return cont (newTypeVarMap) (ctx.value);
      ...
      (newTypeVarMap, x) => {
        if (Z.all (t => t._test (ctx.env) (x), ancestors ($1))) {
          return $1.new ({
            ...
            value: x,
          }) (typeVarMap) (newTypeVarMap) (newTypeVarMap => value => newTypeVarMap);
        } else {
...
const BinaryType = name => url => supertypes => test => _1 => _2 => $1 => $2 => Object.assign (Object.create (Type$prototype), {
  ...
  new: ctx => typeVarMap => newTypeVarMap => cont => {
    ...
    const newNewNewTypeVarMap = Z.reduce (
      (newTypeVarMap, x) => {
    ...
    return cont (newNewNewTypeVarMap) (ctx.value);
  },
});
  ...
  new: ctx => typeVarMap => newTypeVarMap => cont => {
    const newNewTypeVarMap =
      wrapTypeVarMap (newTypeVarMap)
                     (name_ => values => name_ === name ? [...values, {path: key, value: ctx.value}] : values);
          ...
          case 1:
            Z.map (
              x => t.blah.$1.type.new ({
                ...
                propPath: [...ctx.propPath, '$1'],
                value: x,
              }) (typeVarMap) (newTypeVarMap) (newTypeVarMap => value => value),
...
const Function_ = types => Object.assign (Object.create (Type$prototype), {
  ...
  new: ctx => typeVarMap => newTypeVarMap => cont => {
    ...
    return cont (newNewTypeVarMap) ((...args) => {
      ...
      const newNewTypeVarMap = args.reduce ((newTypeVarMap, arg, idx) => {
      ...
      if (Z.all (t => t._test (ctx.env) (returnValue), ancestors (types[types.length - 1]))) {
        return types[types.length - 1].new ({
          ...
          value: returnValue,
        }) (typeVarMap) (newNewTypeVarMap) (newTypeVarMap => value => value);
      } else {
...
const RecordType = fields => {
  ...
  return Object.assign (Object.create (Type$prototype), {
    ...
    new: ctx => typeVarMap => newTypeVarMap => cont => {
      const newNewTypeVarMap = keys.reduce ((newTypeVarMap, k) => {
...
const NamedRecordType = name => url => supertypes => fields => {
  ...
  return Object.assign (Object.create (Type$prototype), {
    ...
    new: ctx => typeVarMap => newTypeVarMap => cont => {
      ...
      return cont (newTypeVarMap) (ctx.value);
    },
  });
};
...
const Fn = $1 => $2 => (
  Object.assign (
    Function_ ([$1, $2]),
    {
      new: ctx => typeVarMap => newTypeVarMap => cont => {
        const $values = ['TK'];
        const newNewTypeVarMap = name => { console.log ('name::', name); return $values; };
        return cont (newNewTypeVarMap) ((...args) => {
          if (args.length !== 1) {
          ...
          if (Z.all (t => t._test (ctx.env) (x), ancestors ($1))) {
            $1.new ({
              ...
              propPath: ['$1', ...ctx.propPath],
              value: x,
            }) (typeVarMap) (newNewTypeVarMap) (newTypeVarMap => value => (console.log ('value:', value), value));
            const y = ctx.value (x);
...
const create = opts => {
  const def = name => constraints => types => impl => {
      ...
      const wrapped = (...args) => {
        ...
        if (Z.all (t => t._test (opts.env) (x), ancestors (types[0]))) {
          ...
          return types[0].new ({
            ...
            propPath: [],
            value: x,
          }) (typeVarMap) (name => []) (newTypeVarMap => value => value);
        }
      };
    ...
    .slice (0, -1)
    .reduceRight (
      (run, input, index) => _typeVarMap => newTypeVarMap => f => {
      ...
      typeVarMap => newTypeVarMap => value => {
