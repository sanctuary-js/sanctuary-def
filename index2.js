...
const validate = env => mappings => type => x => {
  if (!(Z.all (t => t._test (x), ancestors (type)) &&
        type._test (x))) {
...
const NamedRecordType = name => url => supertypes => fields => {
  ...
  return Object.assign (Object.create (Type$prototype), {
    ...
    _test: x => {
      ...
      return keys.every (k => fields[k]._test (x[k]));
    },
    format: outer => K (outer (name)),
    new: reject => resolve => env => typeInfo => index => path => value => mappings => {
      if (value == null) {
        ...
        for (const k in value) delete missing[k];
        if (Z.size (missing) > 0 ||
            keys.some (k => !(fields[k]._test (value[k])))) {
          reject (invalidValue (env, typeInfo, index, path, value));
        } else {
          return reduceRight
                   (cont => key => (
