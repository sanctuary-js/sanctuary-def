...
const RecordType = fields => {
  ...
  return Object.assign (Object.create (Type$prototype), {
    ...
    new: fail => typeVarMap => x => {
      ...
      keys.forEach (k => {
        fields[k].new (propPath => fail ([k, ...propPath])) (typeVarMap) (x[k]);
      });
      return x;
    },
  });
...
const NamedRecordType = name => url => supertypes => fields => {
  ...
  return Object.assign (Object.create (Type$prototype), {
    ...
    new: fail => typeVarMap => x => {
      ...
      if (!(isEmpty (missing))) fail ([]) (x);
      keys.forEach (k => {
        fields[k].new (_ => _ => fail ([]) (x)) (typeVarMap) (x[k]);
      });
      return x;
    },
  });
...
const Either = $1 => $2 => Object.assign (
               ...
               ($2),
  {
    new: fail => typeVarMap => x => {
      ...
      if (x.isLeft) {
        $1.new (propPath => fail (['$1', ...propPath])) (typeVarMap) (x.value);
      } else {
        $2.new (propPath => fail (['$2', ...propPath])) (typeVarMap) (x.value);
      }
      return x;
    },
  }
...
const StrMap = $1 => Object.assign (
              ...
              ($1),
  {
    new: fail => typeVarMap => x => {
      ...
      for (const k of keys) {
        $1.new (propPath => fail (['$1', ...propPath])) (typeVarMap) (x[k]);
      }
      return x;
    },
  }
...
$.Array = $1 => (
  Object.assign (
    $.UnaryType ('Array')
                ...
                ($1),
    {
      new: fail => typeVarMap => xs => {
        if (!(Array.isArray (xs))) fail (['$1']) (xs);

        for (const x of xs) {
          $1.new (propPath => fail (['$1', ...propPath])) (typeVarMap) (x);
        }
...
const Fn = $1 => $2 => (
  Object.assign (
    $.Function ([$1, $2]),
    {
      new: fail => typeVarMap => f => {
        if (typeof f !== 'function') fail ([]) (f);
        return x => {
          const i = $1.new (propPath => fail (['$1', ...propPath])) (typeVarMap) (x);
          const o = $2.new (propPath => fail (['$2', ...propPath])) (typeVarMap) (f (x));
          return o;
        };
      },
    }
