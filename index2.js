...
const _determineActualTypes = (
  ...
  function refine(types, value) {
    ...
    return Z.chain (t => (
      validate (env) (typeInfo) (index) (t) (value) != null ?
        [] :
      t.type === 'UNARY' ?
...
const satisfactoryTypes = (
  ...
  for (let idx = 0; idx < values.length; idx += 1) {
    const result = validate (env) (typeInfo) (index) (expType) (values[idx]);
    if (result != null) {
      return Left (null);
    }
  }
...
const validate = env => typeInfo => index => type => x => {
//ancestors (type)
//.forEach (type => {
//  type.new
//    (error => { throw error; })
...
//});
  ...
  for (const k of Object.keys (type.blah)) {
    ...
    for (const y of ys) {
      const result = validate (env) (typeInfo) (index) (t) (y);
      if (result != null) {
